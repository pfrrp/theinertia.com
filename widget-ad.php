...
class widget_advertisement extends WP_Widget {
	function widget_advertisement() {
		$widget_ops = array('classname' => 'widget_advertisement', 'description' => __( 'An advertisment widget (300x250)') );
		$this->WP_Widget('advertisement', __('Advertisement'), $widget_ops);	
	}
        ...
        <?php }	?>
        	<div class="ad cAdInner">
				...
				<?php if (is_category('surf') || (is_single() && in_category('surf'))) { ?>
					GA_googleFillSlot("300x250_Inertia_Surf_Top_Right");		  
				<?php }elseif (is_category('music-art') || (is_single() && in_category('music-art'))) { ?>
					GA_googleFillSlot("300x250_Inertia_Music_and_Art_Top_Right");
				<?php }elseif (is_category('health') || (is_single() && in_category('health'))) { ?>
					GA_googleFillSlot("300x250_Inertia_Health_Top_Right");
				<?php }elseif (is_category('environment') || (is_single() && in_category('environment'))) { ?>
					GA_googleFillSlot("300x250_Inertia_Environment_Top_Right");
				<?php }elseif (is_category('business-media') || (is_single() && in_category('business-media'))) { ?>
					GA_googleFillSlot("300x250_Inertia_Business_and_Media_Top_Right");
				<?php }elseif (is_category('travel') || (is_single() && in_category('travel'))) { ?>
					GA_googleFillSlot("300x250_Inertia_Travel_Top_Right");
				<?php }elseif (is_category('politics') || (is_single() && in_category('politics'))) { ?>
					GA_googleFillSlot("300x250_Inertia_Politics_Top_Right");
				<?php }elseif (is_category('comedy') || (is_single() && in_category('comedy'))) { ?>
					GA_googleFillSlot("300x250_Inertia_Comedy_Top_Right");
				<?php }elseif (is_page('contributors') || is_author()) { ?>
					GA_googleFillSlot("300x250_Inertia_Contributors_Top_Right");
				<?php }else { ?>
					GA_googleFillSlot("300x250_Inertia_Homepage_Top_Right");				
				<?php } ?>
				/* ]]> */
                </script>       
            </div>
